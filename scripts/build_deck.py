...
CYAN  = RGBColor(0x1E, 0xA7, 0xDD)   # bright azure accent
CYAN2 = RGBColor(0x57, 0xC2, 0xEC)
NAVY  = RGBColor(0x12, 0x30, 0x47)   # title ink
TEAL  = RGBColor(0x0E, 0x3D, 0x4F)
DARK  = RGBColor(0x1E, 0x2C, 0x33)   # body text
GREY  = RGBColor(0x8A, 0x98, 0xA0)
GREYD = RGBColor(0xAE, 0xC2, 0xCC)   # grey on dark
LINE  = RGBColor(0xD9, 0xE2, 0xE7)
PANEL = RGBColor(0xF3, 0xF7, 0xF9)
WHITE = RGBColor(0xFF, 0xFF, 0xFF)
DCARD = RGBColor(0x10, 0x33, 0x44)   # card on dark
...
def header(s, kicker, title_navy, title_cyan, n, dark=False):
    ...
    text(s, 0.92, 0.46, 11, 0.32, [[(kicker.upper(), 11, kcol, FSEMI)]], sp_after=0)
...
text(s, 1.95, 4.32, 9, 0.4, [[('Analisis & Monitoring Data dengan Pendekatan CRISP-DM', 14, DARK, FSEMI)]], sp_after=0)
...
for num, t, d in items:
    notch(s, 0.9, y, 1.0, 0.8, fill=NAVY, line=None, sn=0.18)
...
text(s, 0.9, 1.62, 11.6, 0.7,
     [[('Tim mengangkat ', 14, DARK, FBODY), ('dua studi kasus pengolahan data nyata', 14, CYAN, FSEMI),
       (' dari lingkungan kerja anggota, lalu mengolahnya secara terstruktur (CRISP-DM) hingga menjadi dashboard pemantauan.', 14, DARK, FBODY)]],
...
text(s, 0.9, 1.62, 11.6, 0.4,
     [[('Setiap studi kasus mengikuti enam fase CRISP-DM berurutan, dari pemahaman bisnis hingga deployment dashboard.', 13, GREY, FBODY)]], sp_after=0, line=1.1)
phases = [
    ('1', 'Business\nUnderstanding', 'Tujuan & business questions'),
    ('2', 'Data\nUnderstanding', 'Load data, profil kualitas'),
    ('3', 'Data\nPreparation', 'Cleaning, kolom turunan'),
    ('4', 'Modeling /\nAnalisis', 'Agregasi & visualisasi'),
    ('5', 'Evaluation', 'Temuan & rekomendasi'),
    ('6', 'Deployment', 'Dashboard monitoring'),
]
...
text(s, 0.9, 5.35, 11.6, 0.5,
     [[('Pendekatan ini memastikan analisis selalu berangkat dari pertanyaan bisnis dan diakhiri dengan artefak yang dapat dipantau, bukan sekadar grafik lepas.', 12.5, GREY, FBODY, True)]], sp_after=0, line=1.15)

# ---------- 5. PM intro ----------
n += 1; s = slide(); header(s, 'Manajemen Proyek', 'PELACAKAN PEKERJAAN ', 'GITHUB PROJECTS', n)
text(s, 0.9, 1.62, 11.6, 0.8,
     [[('Pekerjaan kelompok dikelola pada ', 14, DARK, FBODY), ('board kanban GitHub Projects', 14, CYAN, FSEMI),
       (', dengan setiap fase CRISP-DM dari kedua studi kasus dipecah menjadi kartu tugas dan dilacak melalui kolom ', 14, DARK, FBODY),
...
for t, d in why:
    ...
    x += 3.97
notch(s, 0.9, 4.75, 11.53, 1.5, fill=NAVY, line=None, sn=0.05)
...
text(s, rx, yy+0.12, 3.45, 1.6,
     [[('16 kartu total', 13, CYAN, FSEMI)],
      [('Kartu = fase CRISP-DM kedua studi kasus + setup, dokumentasi, dan pengumpulan.', 11, GREY, FBODY)]],
...
for ri, (a,b,c,hd) in enumerate(rows):
    ...
    for ci, val in enumerate((a,b,c)):
        ...
        col = WHITE if hd else (CYAN if ci==0 and not hd else (WHITE if hd else DARK))
...
text(s, 0.9, 1.66, 11.6, 1.0,
     [[('CGS-1 (Custody Gas Station)', 14, CYAN, FSEMI),
       (' memiliki sejumlah instrumen lapangan yang menopang keandalan, keselamatan, dan kepatuhan operasi. Data tersedia namun belum diolah dan kualitasnya belum dievaluasi.', 14, DARK, FBODY)]],
     sp_after=0, line=1.18)
qs = [
...
text(s, 7.35, 5.45, 4.7, 0.9,
     [[('Keterbatasan:  ', 11, CYAN, FSEMI), ('sampel kecil (18 instrumen), satu area, snapshot satu waktu → analisis deskriptif, belum prediktif.', 11, GREY, FBODY)]],
...
text(s, 0.9, 1.66, 11.6, 0.9,
     [[('Sebuah retailer online B2C', 14, CYAN, FSEMI),
       (' ingin memahami kinerja penjualannya selama ~3,5 tahun (data agregat mingguan, anonim) untuk mendukung keputusan stok, promo, dan target.', 14, DARK, FBODY)]],
     sp_after=0, line=1.18)
qs = [
...
text(s, rx, 5.45, 12.4-rx, 0.9,
     [[('Interaktif:  ', 11.5, CYAN, FSEMI), ('hover, zoom, filter tahun, langsung di notebook.', 11.5, GREY, FBODY)]],
     sp_after=0, line=1.12)
